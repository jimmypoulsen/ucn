...
public class NewOrderUI extends JDialog {
	...
	public static void main(String[] args) {
		try {
			NewOrderUI dialog = new NewOrderUI();
	...
	private void newOrder() {
		...
		if(c == null) {
			JOptionPane.showMessageDialog(null, "Customer could not be found .. Try again!", "Error", JOptionPane.ERROR_MESSAGE);
			txtCustomerNo.setText("");
		} else {
			order = orderCtrl.newOrder(c);
	...
	private void createOrder() {
		if(order.getOrderLines().size() > 0) {
			...
			int paymentTypeOption = JOptionPane.showOptionDialog(null, "How will the customer pay?", "Select payment type", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[2]);
			...
			if(orderCtrl.createOrder(order, pt) != null) {
				JOptionPane.showMessageDialog(null, "A new order was created!", "Order created", JOptionPane.INFORMATION_MESSAGE);
			...
			HashMap<Integer, Order> orders = new HashMap<Integer, Order>(orderCtrl.getOrders());
			orders.values().forEach(order -> {
				System.out.println(order);
			});
			this.setVisible(false);
		} else
			JOptionPane.showMessageDialog(null, "You need to add at least one order line, in order to create an order.", "No order lines", JOptionPane.ERROR_MESSAGE);
	}
	...
	private void addOrderLine() {
		...
		if(i != null) {
			order.addOrderLine(new OrderLine(i, Integer.parseInt(txtItemQuantity.getText())));
			txtItemBarcode.setText("");
			txtItemQuantity.setText("");
			System.out.println(order);
		}
